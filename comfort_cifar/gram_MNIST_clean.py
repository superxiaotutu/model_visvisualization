...
def save():
    ...
    saver.save(sess, "model1_0.5_1/model.ckpt")


def restore():
    saver = tf.train.Saver()
    saver.restore(sess, "model1_0.5_1/model.ckpt")
...
for ii, hp_a in enumerate(hplist):
    ...
    for j, hp_b in enumerate(hplist):
        for k, hp_c in enumerate(hplist):
            ...
            #     cam_rar = np.reshape(R_cam[i], [28, 28])
            #     mask_rar = np.reshape(R_mask[i], [28, 28])
            #
            #     img_adv = np.reshape(ADV_s[i], [28, 28])
            #     cam_adv = np.reshape(A_cam[i], [28, 28])
            #     mask_adv = np.reshape(A_mask[i], [28, 28])
            #
            #     plt.figure()
            #     plt.subplot(1, 6, 1)
            #     plt.imshow(img_rar)
