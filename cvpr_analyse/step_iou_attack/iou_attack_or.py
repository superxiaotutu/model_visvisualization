...
def get_rar_adv_iou(rar, adv):
    sum = tf.add(rar, adv)
    count_no_0 = tf.count_nonzero(sum,axis=1)
    temp_sub_2 = tf.subtract(sum, constant_2)
    temp_sub_2_nonzero = tf.count_nonzero(temp_sub_2,axis=1)
...
rar_adv_iou=get_rar_adv_iou(rar_grad_cam, adv_grad_cam)
...
for i in range(50000):
    ...
    print(sess.run(rar_adv_iou,feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 1.0}).shape)
    print(sess.run(rar_adv_iou,feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 1.0}))
    print(sess.run(is_iou_exceeds, feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 1.0}))
    break
