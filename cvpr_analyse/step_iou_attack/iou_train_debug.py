...
def save():
    ...
    saver.save(sess, "model_iou_" + str(key) + "_" + str(a) + '_' + str(b) + '_' + str(c) + "/model.ckpt")
...
result_file = 'result_iou_' + str(key) + '_attack_' + str(a) + '_' + str(b) + '_' + str(c) + '.txt'
# sess.graph.finalize()
...
for i in range(50000):
    ...
    iou = np.ones(_BATCH_SIZE)

    adv_sample = sess.run(fixed_adv_sample_get_op, feed_dict={X: batch[0], Y: batch[1], keep_prop: 1.0, IOUS: iou})
    ...
    show_img(batch[0][0], show_rar, show_adv)    # show_img(batch[0][0], rar[0], adv[0])
    ...
    while True:
        if step == 4:
            break
        adv_sample = sess.run(fixed_adv_sample_get_op, feed_dict={X:adv_sample, Y: batch[1], keep_prop: 1.0, IOUS: iou})
        adv = sess.run(adv_cam, feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 1.0, IOUS: iou})
        ious = sess.run(rar_adv_ious, feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 1.0})

        print(ious)

        show_img(batch[0][0], rar, adv)
        _, acc, summery = sess.run([train_op, accuracy, summary_op],
                                   feed_dict={X: batch[0], X_adv: adv_sample, Y: batch[1], keep_prop: 0.5})
        step += 1
