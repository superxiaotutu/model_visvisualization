...
pred_label=tf.argmax(rar_probs, 1)
    ...
    with open(labels_file, 'r', encoding='utf-8')as f:
        try:
            ...
            for index, line in enumerate(lines):
                ...
                pred_labels=sess.run(pred_label,feed_dict={X: imgs, Y: labels})
                adv_labels=sess.run(adv_label,feed_dict={X: imgs, Y: labels})
                true_label=labels
                for j in range(_BATCH_SIZE):
                    rar_l=pred_labels[j]
                    adv_l=adv_labels[j]
                    print(true_label[j],rar_l,adv_l)
                    if true_label[j]==rar_l:
                        np.savez(
                            "npz/" + str(rar_l) + "_" + str(adv_l) + "_" + str(
                                j),imgs[j], rar_maps[j], adv_maps[j], ground_truths[j])
