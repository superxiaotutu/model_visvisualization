        ...
        for index, line in enumerate(lines):
            ...
            with open(results_file, 'a', encoding='utf-8') as f_w:
                for j in range(_BATCH_SIZE):
                    ...
                    true_label =labels[j]
                    if rar_label == true_label:

                        rar_count, adv_count, rar_adv_iou = get_rar_adv_iou(make_cam(rar_maps[j]),
                                                                            make_cam(adv_maps[j])
                                                                            )
                        # plt.figure()
                        # plt.subplot(1, 4, 1)
                        # plt.imshow(imgs[j])
                        # plt.subplot(1, 4, 2)
                        # plt.imshow(make_cam(rar_maps[j]))
                        # plt.subplot(1, 4, 3)
                        # adv_map = make_cam(adv_maps[j])
                        # plt.imshow(adv_map)
                        # plt.subplot(1, 4, 4)
                        ...
                        # plt.close()
                        #
                        f_w.write(str(rar_label) + " " + str(adv_label) + "\n")

                        if is_defenses[j]:
                            defense_iou += rar_adv_iou
                            defense_count += 1
                            continue
                        else:
                            rar_count, adv_count, gt_count, rar_g_iou, adv_g_iou = get_iou(make_cam(rar_maps[j]),
                                                                                           make_cam(adv_maps[j]),
                                                                                           ground_truths[j])
                            if adv_g_iou == 'False' or rar_g_iou == 0 or adv_g_iou == 1 :
                                continue

                            attack_iou += rar_adv_iou
                            attack_count += 1
                            ...
                            print(loop_num)
                            print(rar_label, adv_label, rar_g_iou, adv_g_iou, rar_adv_iou)

    with open(results_file, 'a', encoding='utf-8') as f_w:
        f_w.write('rar' + " " + str(rar_ground_iou_sum / loop_num) + "\n")
        f_w.write('adv' + " " + str(adv_ground_iou_sum / loop_num) + "\n")
        f_w.write('defense_iou' + " " + str(defense_iou / defense_count) + "\n")
        f_w.write('attack_iou' + " " + str(attack_iou / attack_count) + "\n")
