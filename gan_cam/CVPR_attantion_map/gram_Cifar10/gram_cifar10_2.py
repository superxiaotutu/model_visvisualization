...
for i in range(128000):
    ...
    _, adv_acc = sess.run([train_op, accuracy],
    ...
    if i % 1000 == 0:
        print(acc, adv_acc)
        with open('c_loss.txt', 'a')as f:
            f.write(str(i) +' '+ str(acc) +' '+ str(adv_acc)+"\n")
...
for i in range(20):
    img_rar = np.reshape(batch[0][i], [32, 32, 3])
    cam_rar = np.reshape(R_cam[i], [32, 32])
...
print(adv_ACC / 200)
with open('c_loss.txt','a')as f:
    f.write("进行RAR测试集测试:"+str(ACC / 200)+str(adv_ACC / 200))
