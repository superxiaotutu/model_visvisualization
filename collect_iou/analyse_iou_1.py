        ...
        for index, line in enumerate(lines):
            ...
            with open(results_file, 'a', encoding='utf-8') as f_w:
                for j in range(_BATCH_SIZE):
                    ...
                    rar_count, adv_count,rar_g_iou,adv_g_iou  =get_iou(rar_maps[j], adv_maps[j], ground_truths[j])
                    IOU =get_iou2(rar_maps[j], adv_maps[j])
                    ...
                    f_w.write(
                        str(loop_num) +" "+" "+ str(is_defense[j]) + ' ' + str(attack_iou) + ' ' + str(defense_count)
                        +' ' + str(rar_ground_iou_sum) + str(adv_ground_iou_sum) + ' ' + ' ' + str(
                            IOU) + '\n')
    with open(results_file, 'a', encoding='utf-8') as f_w:
        ...
        f_w.write('adv' + str(adv_ground_iou_sum / attack_iou)+"\n")
