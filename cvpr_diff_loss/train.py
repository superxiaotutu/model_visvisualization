...
loss_c = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits)-tf.nn.softmax_cross_entropy_with_logits(labels=Y, logits=logits_c), name='loss_c')
...
while not coord.should_stop():
    ...
    _, loss_num, acc_num, summary_str = sess.run([train, loss, accuracy, summary_op], feed_dict={X: one_of_train_batch[1], Y: one_of_train_batch[0]})
    train_writer.add_summary(summary_str, i)
    print('Iter: {}, LOSS: {}, ACC: {}'.format(i, loss_num, acc_num))
    i += 1
    if i % 10 == 0:
        one_of_test_batch = sess.run(test_batch)
        _, test_summary = sess.run([accuracy, summary_op], feed_dict={X: one_of_test_batch[1], Y: one_of_test_batch[0]})
